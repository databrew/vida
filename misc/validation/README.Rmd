---
title: "Babel validation exercise"
...
```{r}
#For Kenya, if you and Xing could create a single file which contains all the unique deaths (from all the Kenya files) and as many as possible mapped variables then we can start trying to do some basic summaries to understand if the numbers look reasonable for Kenya. We know for some variables that they should come out in very high numbers and some much lower. We will work to compile a list of variables which are worth looking at.
```
...
```{r, echo = TRUE, fig.height=7, eval = FALSE}
